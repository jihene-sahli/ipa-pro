...
function ImageMapper() {
	...
	var app = (function() {
		...
		return {
			...
			getJSONCode : function(arg, highlight) {
				var json_code = {};
				json_code.areas = [];
				if (arg) {
					if (!objects.length) {
					...
					json_code.name = 'map';
					json_code.src = 'images/' + filename;
					utils.foreachReverse(objects, function(x) {
						json_code.areas.push(x.toJSON());
					});
				} else {
					utils.foreachReverse(objects, function(x) {
						json_code.areas.push(x.toJSON());
				...
				if(highlight)
					return this.syntaxHighlight(json_code);
				else	
					return json_code;
			},
	...
	var code = (function(){
		...
		return {
			...
			printJSON: function() {
				content.innerHTML = '<pre>' + app.getJSONCode(true, true) + '</pre>';
				utils.show(block);
			},
			hide: function() {
				utils.hide(block);
